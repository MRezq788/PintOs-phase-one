...
thread_init (void) 
{
  ...
  if (thread_mlfqs)
  {
      initial_thread->nice = 0; //// it is an integer
      initial_thread->recentCpu = 0; /// it is FP
      loadAvg = 0; /// it is FP
  }
}
...
thread_tick (void) 
{
  ...
  if (thread_mlfqs)
  {
      mlfqs(timer_ticks(), 100, thread_current());
  }
...
// dewe start
bool 
compare_threads_piriority(const struct list_elem *a,const struct list_elem *b,void *aux UNUSED) 
{
...
void
thread_set_nice (int nicee UNUSED) 
{
  enum intr_level old_level = intr_disable(); // get old state of the interrupt

  thread_current()->nice = nicee;
  int maxpriority = convertToFP(PRI_MAX);
  int recCpu = ((thread_current()->recentCpu) / 4);
  int niceval = multiply_thenconvert(thread_current()->nice, 2);
  thread_current()->priority = maxpriority - recCpu - niceval;
  ...
  {
    elment = list_begin(&ready_list);
    struct thread *max_priority_thread = list_entry(elment, struct thread, readyelem);   //after calc priority if the current not the highest priority yield it
...
{
   return convertTOInt(loadAvg * 100);
}
...
{
    return convertTOInt(thread_current()->recentCpu * 100);
}

